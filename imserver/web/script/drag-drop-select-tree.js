...
************************************************************************************************************/
	...
	function JSDragDropTree()
	{
		...
		this.filePathselectItem = 'selectItem.do';
	...
	JSDragDropTree.prototype = {
		...
		,
		setFileNameSelect : function(newFileName)
		{
			this.filePathselectItem = newFileName;
		}
		,
		...
		*/
				...
				while(li){
					if(li.id){
						...
						if(numericID!='0'){
							...
							//saveString = saveString + li.id;
							saveString = saveString + '-';


							if(li.parentNode.id!=this.idOfTree)saveString = saveString + numericParentID; else saveString = saveString + '0';
							//if(li.parentNode.id!=this.idOfTree)saveString = saveString + li.parentNode.id; else saveString = saveString + '0';
						}
		...
		,
		selectItem : function(obj1,obj2)
		{
			var rid=parent.getroleid();
			if ("-1"!=rid){
				var message = '确定添加 ' + obj2.innerHTML ;
				if(this.hasSubNodes(obj2.parentNode)) message = message + '及其子项目';
				message = message + ' 到会议中吗？'
				if(confirm(message)){
					this.__selectItem_step2(obj2.parentNode);	// Sending <LI> tag to the __selectItem_step2 method
				}
			}
			else {
				var message = '请先选择加入成员的角色';
				alert(message);
			}
		}
		,
		...
		,
		__selectItem_step2 : function(obj)
		{

			var saveString = obj.id.replace(/[^0-9]/gi,'');

			// since the server can delete children automatically,
			// so we don't need to add children to the list
			...
			JSTreeObj.ajaxObjects[ajaxIndex].setVar("selectId", saveString);
			var rid=parent.getroleid();
			JSTreeObj.ajaxObjects[ajaxIndex].setVar("rid",rid );
			//JSTreeObj.__addAdditionalRequestParameters(JSTreeObj.ajaxObjects[ajaxIndex], JSTreeObj.additionalDeleteRequestParameters);
			JSTreeObj.ajaxObjects[ajaxIndex].requestFile = JSTreeObj.filePathselectItem;	// Specifying which file to get
			JSTreeObj.ajaxObjects[ajaxIndex].onCompletion = function() { JSTreeObj.__selectComplete(ajaxIndex,obj); } ;	// Specify function that will be executed after file has been found
			JSTreeObj.ajaxObjects[ajaxIndex].onLoading = function() { JSTreeObj.__showMessage(); } ;
		...
		,
		__selectComplete : function(ajaxIndex,obj)
		{
			parent.adduserstolist(this.ajaxObjects[ajaxIndex].response);
			messageObj.close();
		}
		,
		__renameComplete : function(ajaxIndex)
		{
			if(this.ajaxObjects[ajaxIndex].response!='OK'){
				alert('重命名项目出错: ' + this.ajaxObjects[ajaxIndex].response); 	// Rename failed
			}
		}
		,
		...
		initTree : function()
		{
			...
			messageObj.setHtmlContent("正在处理，请稍候……");
			...
			if(this.deleteAllowed || this.renameAllowed){
				try{
					...
					if(this.deleteAllowed)menuModel.addItem(1,'加入会议','','',false,'JSTreeObj.selectItem');
					...
					if(this.deleteAllowed)menuModelDeleteOnly.addItem(4,'加入会议','','',false,'JSTreeObj.selectItem');
			...
			for(var no=0;no<menuItems.length;no++){
				...
				if(menuItems[no].className){
					...
					if(!noChildren)
						...
						folderImg.src = this.imageFolder + this.nodeImage;
				}
