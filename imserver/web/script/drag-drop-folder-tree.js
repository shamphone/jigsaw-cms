...
************************************************************************************************************/
	...
	JSDragDropTree.prototype = {
		...
		*/
		...
		,
		resetItem : function(obj1,obj2)
		{
			var uid = obj2.parentNode.id.replace(/[^0-9]/gi,'');
			window.location="/Webmanager/usermanage/setUserPasswordPage.do?uid="+uid;
			//,"setuserpass","height=220,width=360,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no");
		}
		,
		viewItem : function(obj1,obj2)
		{
			var uid = obj2.parentNode.id.replace(/[^0-9]/gi,'');
			window.location="/Webmanager/usermanage/viewInfo.do?uid="+uid;
		}
		,
		deleteItem : function(obj1,obj2)
		{
			var message = '按 确定 删除项目' + obj2.innerHTML;
		...
		__deleteItem_step2 : function(obj)
		{
			...
			JSTreeObj.ajaxObjects[ajaxIndex].setVar("deleteId", saveString);
		...
		__deleteComplete : function(ajaxIndex,obj)
		{
			...
			if(this.ajaxObjects[ajaxIndex].response!='OK'){
				var contents=this.ajaxObjects[ajaxIndex].response;
				showModalDialog("about:"+contents+"","","dialogHeight:120px;dialogWidth:240px;help:no;center:yes;status:no;resizable:No")
				//alert('删除项目出错: ' + this.ajaxObjects[ajaxIndex].response); 	// Rename failed
			}else{
		...
		initTree : function()
		{
			...
			if(this.deleteAllowed || this.renameAllowed){
				try{
					...
					menuModel.addItem(2,'详细信息','','',false,'JSTreeObj.viewItem');
					...
					menuModelDeleteOnly.addItem(4,'重置密码','','',false,'JSTreeObj.resetItem');
					menuModelDeleteOnly.addItem(5,'详细信息','','',false,'JSTreeObj.viewItem');
					if(this.deleteAllowed)menuModelDeleteOnly.addItem(6,'删除','','',false,'JSTreeObj.deleteItem');
