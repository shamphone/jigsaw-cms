///////////////////////////////
// Message definition file
// Generated by lyvc Message
///////////////////////////////

package com.fulong.lyvc.message;

/**
 * ConferenceModelRoleTableItem
 * 
 * 龙驭视频会议系统 v3.0
 * 
 * 版权所有：北京中科辅龙计算机技术股份有限公司 2009
 * 
 * @author 李雄锋
 * 
 * 最后修改时间：2010-9-10
 */

public class ConferenceModelRoleTableItem extends BaseMessage{

	public static final int id = 54;

	public String modelId;
	public String roleId;
	public String roleName;
	public String rights;

	public String toXML() {
		StringBuffer xml = new StringBuffer();
		xml.append("<lyvcmessage>");
		xml.append("<id>" + id + "</id>");
		xml.append("<_senderid>" + this.getSenderId() + "</_senderid>");
		xml.append("<_conferenceid>" + this.getConferenceId() + "</_conferenceid>");
		xml.append("<modelId>" + this.modelId + "</modelId>");
		xml.append("<roleId>" + this.roleId + "</roleId>");
		xml.append("<roleName>");
		if(this.roleName != null) {
			xml.append(encodeHtmlEscape(this.roleName));
		}
		xml.append("</roleName>");
		xml.append("<rights>");
		if(this.rights != null) {
			xml.append(encodeHtmlEscape(this.rights));
		}
		xml.append("</rights>");
		xml.append("</lyvcmessage>");
		
		return xml.toString();
	}

	public void fromXML(String xmlString) throws MessageFormatException {
		String tagContent;
		
		tagContent = BaseMessage.getStringBetweenTag(xmlString, "_senderid");
		this.setSenderId(tagContent);
		tagContent = BaseMessage.getStringBetweenTag(xmlString, "_conferenceid");
		this.setConferenceId(tagContent);
		tagContent = BaseMessage.getStringBetweenTag(xmlString, "modelId");
		this.modelId = tagContent;
		tagContent = BaseMessage.getStringBetweenTag(xmlString, "roleId");
		this.roleId = tagContent;
		this.roleName = BaseMessage.decodeHtmlEscape(BaseMessage.getStringBetweenTag(xmlString, "roleName"));
		this.rights = BaseMessage.decodeHtmlEscape(BaseMessage.getStringBetweenTag(xmlString, "rights"));
	}

}
