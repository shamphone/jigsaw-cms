	...
	public void listContextJsps(Context context, Summary summary, boolean compile) throws Exception {
        ...
        if (servletConfig != null) {
            synchronized (servletConfig) {
                ...
                JspRuntimeContext jrctx = new JspRuntimeContext(sctx, opt);
                try {
                    if (summary.getItems() == null) summary.setItems(new HashMap<Object, Item>());

                    //
                    // mark all items as missing
                    //
                    for (Iterator<String> it = summary.getItems().keySet().iterator(); it.hasNext();) {
            ...
            //
            Map<Object, Item> hashMap = new HashMap<Object, Item>();
            for (Iterator<String> it = summary.getItems().keySet().iterator(); it.hasNext();) {
                Object key = it.next();
