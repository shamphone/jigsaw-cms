...
import org.jstripe.tomcat.probe.beans.ResourceResolver;
...
public class ApplicationUtils {
	...
	public static Application getApplication(Context context,
		...
		if (resourceResolver != null) {
			...
			int ctxAttrCount = 0;
			for (Enumeration e = context.getServletContext()
					.getAttributeNames(); e.hasMoreElements(); e.nextElement()) {
				ctxAttrCount++;
			}
			...
			if (resourceResolver.supportsPrivateResources()
					&& app.isAvailable()) {
				app.setDataSourceUsageScore(ApplicationUtils
						.getApplicationDataSourceUsageScore(context,
								resourceResolver));
			}
		}
	...
	@SuppressWarnings("unchecked")
	public static int getApplicationDataSourceUsageScore(Context context,
		...
		for (Iterator it = appResources.iterator(); it.hasNext();) {
			...
			if (appResource.getDataSourceInfo() != null) {
				score = Math.max(score, appResource.getDataSourceInfo()
						.getScore());
			}
		}
		return score;
	...
	public static ApplicationSession getApplicationSession(Session session,
			boolean calcSize, boolean addAttributes) {
		...
		if (session != null && session.isValid()) {
			...
			List<Object> processedObjects = new ArrayList<Object>(1000);
			try {
				for (Enumeration<?> e = httpSession.getAttributeNames(); e
					...
					if (calcSize) {
						try {
							...
							logger.error("Cannot estimate size of attribute \""
									+ name + "\"", th);
							//
				...
				String lastAccessedIP = (String) httpSession
						.getAttribute(ApplicationSession.LAST_ACCESSED_BY_IP);
				if (lastAccessedIP != null) {
					sbean.setLastAccessedIP(lastAccessedIP);
				}
				try {
					sbean.setLastAccessedIPLocale(InetAddressLocator
							.getLocale(InetAddress.getByName(lastAccessedIP)
									.getAddress()));
				} catch (Throwable e) {
					logger
							.error("Cannot determine Locale of "
									+ lastAccessedIP);
				}
	...
	public static List<Attribute> getApplicationAttributes(Context context) {
		...
		for (Enumeration<?> e = servletCtx.getAttributeNames(); e
				.hasMoreElements();) {
	...
	public static List<ApplicationParam> getApplicationInitParams(
			Context context) {
		...
		ServletContext servletCtx = context.getServletContext();
		for (Enumeration<?> e = servletCtx.getInitParameterNames(); e
				.hasMoreElements();) {
	...
	public static ServletInfo getApplicationServlet(Context context,
			String servletName) {
	...
	private static ServletInfo getServletInfo(Wrapper w) {
		...
		if (w instanceof StandardWrapper) {
			...
			si.setMinTime(sw.getMinTime() == Long.MAX_VALUE ? 0 : sw
					.getMinTime());
	...
	public static FilterInfo getApplicationFilter(Context context,
			String filterName) {
	...
	public static List<FilterMapping> getApplicationFilterMaps(Context context)
			throws Exception {
		FilterMap[] fms = context.findFilterMaps();
		List<FilterMapping> filterMaps = new ArrayList<FilterMapping>(
				fms.length);
		for (int i = 0; i < fms.length; i++) {
			if (fms[i] != null) {
				FilterMapping fm = new FilterMapping();
				// FilterMap getURLPattern() was replaced with getURLPatterns()
				// in Tomcat 6
				// the following line results in an exception when Probe runs on
				// Tomcat 6 !!!
				// TODO: ApplicationUtils needs to be refactored in order to
				// become Tomcat version specific
				Method getURLPatternMethod = FilterMap.class
						.getMethod("getURLPattern");
				String urlPattern = (String) getURLPatternMethod.invoke(fms[i]);
				fm.setUrl(urlPattern);
				Method getServletNameMethod = FilterMap.class
						.getMethod("getServletName");
				String servletName = (String) getServletNameMethod
						.invoke(fms[i]);
