...
<ul class="options">
    ...
    <c:if test="${! empty content || ! empty highlightedContent}">
        <c:if test="${item.state == 2}">
            <li id="viewservlet">
                <a href="<c:url value="/system/probe/app/viewservlet.htm">
                        <c:param name="webapp" value="${param.webapp}"/>
                        <c:param name="source" value="${param.source}"/>
                    </c:url>"><spring:message code="probe.jsp.viewsource.opt.viewServlet"/></a>
            </li>
        </c:if>
        <li id="compilesingle"><a
                href="<c:url value="/system/probe/app/recompile.htm"><c:param name="webapp" value="${param.webapp}"/><c:param name="source" value="${param.source}"/><c:param name="view" value="/system/probe/app/viewsource.htm"/></c:url>">
