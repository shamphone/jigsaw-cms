 ...
 * @version 3.0
 */
public class ReferenceValueTag extends SpringTagSupport {
    /**
	 * 
	 */
	private static final long serialVersionUID = 5493710276910268355L;
	...
	public int doStartTag() throws JspException {
        ...
        Property property = thisContent.getProperty("post-content-ref");
        if(property!=null){
            Node node = property.getReference();
            if(node!=null){
                Property oProperty = node.getProperty(this.property);
                if(oProperty!=null){
                    value.append(this.formatProperty(oProperty));
                } else
                    try {
                        oProperty = (Property) PropertyUtils.getProperty(node,
                                this.property);
                        value.append(formatValue(oProperty));
                    } catch (Exception ex) {
                        if (!this.ignore){
            ...
            throw new JspException("Unable to find reference!");
        }
                ...
                switch (type) {
                    ...
                    case PropertyType.REFERENCE: //引用属性,目前仅处理resource；
                        ...
                        if (resource != null) {
                            ...
                            Node content = this.getRepository().getNode(values[i].getString());
                            if (content != null)
                    ...
                    /*
                        if (property.getDefinition().getEnumEntry() != null) {
                            Entry entry = this.getDictManager().getRootEntry().
                                getEntry(property.getDefinition().getEnumEntry());
                            if (entry != null)
