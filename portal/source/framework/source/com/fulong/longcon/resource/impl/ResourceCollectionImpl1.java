 ...
 * @version 1.0
 */
public class ResourceCollectionImpl1 implements ResourceCollection {
	...
	public ResourceCollectionImpl1(ResourceManagerExt repository) {
	...
	public void filterByParent(String path, boolean recursive) {
		if (path != null) {
			if (recursive) {
				this.recursiveWhere
						.append(" start with PARENT_PATH=? connect by prior PATH=PARENT_PATH");
	...
	public void filterByOwner(Principal owner) {
		if (owner != null) {
			...
			this.parameters
					.add(new SQLParameter(Types.VARCHAR, owner.getName()));
		}
	...
	public void filterByCreator(Principal creator) {
		if (creator != null) {
			...
			this.parameters.add(new SQLParameter(Types.VARCHAR, creator
					.getName()));
		}
	}
	...
	public ResourceIterator resources() {
		...
		SQLParameter[] parameters = (SQLParameter[]) this.parameters
				.toArray(new SQLParameter[this.parameters.size()]);

		ResourceIterator result = new BasicResourceIterator(this.repository,
				parameters, query.toString(), count.toString());
		return result;
