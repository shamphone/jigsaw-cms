 ...
 * @version 2.0
 */
public class OracleSiteCategoryDao extends JdbcDao implements SiteCategoryDao {
	private static final Log log = LogFactory
			.getLog(OracleSiteCategoryDao.class);

	private static final String SQL_FIND_BY_ID = "select * from SITE_CATEGORY where PKID = ? or NAME =?";
	...
	public SiteCategoryData findByID(String ID) throws SQLException {
		...
		try {
			...
			SiteCategoryData[] result = retrieve(statement, SQL_FIND_BY_ID);
			if (result.length > 0) {
				return result[0];
			}
			return null;
	...
	private static final String SQL_GET_ALL = "select * from site_category t order by t.create_time desc";

	public SiteCategoryData[] getAllCategories() throws SQLException {
		PreparedStatement statement = null;
		try {
			statement = connection.prepareStatement(SQL_GET_ALL);
			SiteCategoryData[] result = retrieve(statement, SQL_GET_ALL);
			return result;
		} finally {
			close(statement);
		}
	...
	private SiteCategoryData[] retrieve(PreparedStatement statement, String sql)
			throws SQLException {
		long start = 0;
		...
		rs.close();
		statement.close();
		return (SiteCategoryData[]) result.toArray(new SiteCategoryData[result
				.size()]);
	}

	protected void populate(ResultSet rs, SiteCategoryData data)
			throws SQLException {
	...
	private static final String SQL_Insert = "insert into SITE_CATEGORY (PKID, NAME, DISPLAY_NAME, DESCRIPTION, CREATE_TIME, START_DATE, EXPIRY_DATE, STATE, GROUPID)"
			+ "values (?, ?, ?, ?, ?, ?, ?, ?,?)";
	...
	public void insert(SiteCategoryData data) throws SQLException {
		...
		try {
			...
			if (data.getCreateDate() != null)
				statement.setTimestamp(5, toTimestame(data.getCreateDate()));
			else
				statement.setTimestamp(5, toTimestame(new Date()));
			if (data.getStartDate() != null)
				statement.setTimestamp(6, toTimestame(data.getStartDate()));
			else
				statement.setTimestamp(6, null);
			if (data.getExpiryDate() != null)
				statement.setTimestamp(7, toTimestame(data.getExpiryDate()));
			else
	...
	private static final String SQL_UPDATE = "update SITE_CATEGORY set NAME = ?, DISPLAY_NAME = ?, DESCRIPTION = ?, CREATE_TIME = ?, START_DATE = ?, EXPIRY_DATE = ?, STATE = ?, GROUPID = ?"
			+ " where PKID = ?";
	...
	public void update(SiteCategoryData data) throws SQLException {
		...
		try {
			...
			statement.setTimestamp(4, toTimestame(data.getCreateDate()));
	...
	private static final String SQL_Delete_By_ID = "delete from SITE_CATEGORY where PKID=?";
	 ...
	 * proc = null; try { proc =
	 * connection.prepareCall("{ call pro_delete_SiteCategory(?) }");
	 * proc.setString(1, ID); proc.execute(); } finally { proc.close(); } }
	 */
	public SiteCategoryData[] search(String query, SQLParameter[] parameters,
		...
		try {
			...
			SiteCategoryData[] datas = retrieve(statement, sql);
			return datas;
		} catch (SQLException ex) {
