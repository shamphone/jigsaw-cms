 ...
 * @version 2.0
 */
public class OracleSiteDao extends JdbcDao implements SiteDao {
	private static final Log log = LogFactory.getLog(OracleSiteDao.class);
	...
	public void insert(SiteData data) throws SQLException {
		...
		try {
			...
			if (data.getCreateDate() != null)
				statement.setTimestamp(9, toTimestame(data.getCreateDate()));
			else
	...
	// "delete from SITE where pkid = ?";

	private static final String SQL_DELETE = "update SITE set DEL_FLAG='1' where pkid = ?";
	...
	public void update(SiteData data) throws SQLException {
		...
		try {
			...
			if (data.getCreateDate() != null)
				statement.setTimestamp(8, toTimestame(data.getCreateDate()));
			else
				statement.setTimestamp(8, toTimestame(new Date()));
			if (data.getStartDate() != null)
				statement
						.setTimestamp(9, toTimestame(data.getStartDate()));
			else
			...
			if (data.getExpiryDate() != null)
				...
				statement.setTimestamp(10, null);
			// statement.setDate(8, toSQLDate(data.getCreateDate()));
			// statement.setDate(9, toSQLDate(data.getStartDate()));
			// statement.setDate(10, toSQLDate(data.getExpiryDate()));
