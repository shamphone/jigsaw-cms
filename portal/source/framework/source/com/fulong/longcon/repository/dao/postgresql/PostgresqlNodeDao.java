...
public class PostgresqlNodeDao extends NodeDao{
    ...
    public long countRecByParent(String parentID) throws SQLException {
        ...
        String SQL_Rec_count_By_Parent =
            "select count(PKID) from node n where pkid in ";
        if(parentID==null||parentID.equalsIgnoreCase(""))
        	return 0l;
        SQL_Rec_count_By_Parent =SQL_Rec_count_By_Parent +" ( "+getClauseForRecursivePostgresql("node","pkid","parent_id",null,parentID,"pkid") +" )";
    ...
    public long countRecByParent(String parentID, String name) throws
        ...
        String SQL_Rec_count_By_ParentAndName =
            "select count(PKID) from node n where  n.name=? AND n.pkid in ";
        if(parentID==null || parentID.equals(""))
        	return 0;
        SQL_Rec_count_By_ParentAndName =SQL_Rec_count_By_ParentAndName +" ( "+getClauseForRecursivePostgresql("node","pkid","parent_id",null,parentID,"pkid") +" )";
        PreparedStatement command = null;
        long result;
        try {
    ...
    private static final String SQL_Find_By_Parent =
        "select distinct * from NODE where PARENT_ID=? order by orderno";
    public NodeData[] findByParent(String parentID, long fromIndex, long number) throws
        SQLException {
        PreparedStatement command = null;
        String sql = toRangeQueryForPostgresql(SQL_Find_By_Parent);
        try {
    ...
    public NodeData[] findByParentRec(String parentID, long fromIndex,
        ...
        String SQL_Find_By_ParentRec =
            "select distinct * from node n where  n.pkid in "; 
        if(parentID==null ||parentID.equals(""))
        	return null;
        SQL_Find_By_ParentRec = SQL_Find_By_ParentRec +" ( "+getClauseForRecursivePostgresql("node", "pkid", "parent_id", null, parentID,"pkid")+" ) ";

        PreparedStatement command = null;
        String sql = toRangeQueryForPostgresql(SQL_Find_By_ParentRec);
        try {
            command = connection.prepareStatement(sql);
    ...
    public NodeData[] findByParentRec(String parentID, String name,
        ...
        String SQL_Find_By_ParentAndNameRec =
            "select distinct * from node n where  n.name=? and n.pkid in  "; 
    	if(parentID==null ||parentID.equals(""))
        	return null;
    	SQL_Find_By_ParentAndNameRec = SQL_Find_By_ParentAndNameRec +" ( "+getClauseForRecursivePostgresql("node", "pkid", "parent_id", null, parentID,"pkid")+" ) ";
        PreparedStatement command = null;
        String sql = toRangeQueryForPostgresql(SQL_Find_By_ParentAndNameRec);
        try {
            command = connection.prepareStatement(sql);
            command.setString(1, name);
            command.setLong(2, fromIndex);
    ...
    public NodeData[] search(String query,
                             ...
                             int number) throws
        SQLException {
        String sql = toRangeQueryForPostgresql(query);
    ...
    private static final String SQL_Find_By_ParentAndName =
        "select distinct * from NODE where PARENT_ID=? and NAME = ? order by ORDERNO";
    /**
     * 获取制定父节点下的子节点
     * @param parentID String
     * @param name String
    ...
    public NodeData[] findByParent(String parentID, String name, long fromIndex,
        ...
        SQLException {
        PreparedStatement command = null;
        String sql = toRangeQueryForPostgresql(SQL_Find_By_ParentAndName);
        try {
            command = connection.prepareStatement(sql);
            command.setString(1, parentID);
    ...
    public long countByRecParent(String parentID, String name) throws SQLException {
        ...
        String SQL_count_By_ParentAndName_Recursion =
            "select count(PKID) from node where name=? AND pkid in ";
        if(parentID==null || parentID.equals(""))
        	return 0l;
        SQL_count_By_ParentAndName_Recursion =SQL_count_By_ParentAndName_Recursion 
                                            +" ( "+getClauseForRecursivePostgresql("node","pkid","parent_id",
                                            		                           null,parentID,"pkid") +" )";
        PreparedStatement command = null;
        try {
    ...
    public String[] childrenIDs(String parentID) throws SQLException {
    	String SQL_FindID_By_ParentRec =
            "select pkid from node n where  n.pkid in ";
    	if(parentID==null ||parentID.equals(""))
        	return null;
    	SQL_FindID_By_ParentRec = SQL_FindID_By_ParentRec +" ( "+getClauseForRecursivePostgresql("node", "pkid", "parent_id", null, parentID,"pkid")+" ) ";        
        PreparedStatement command = null;
        try {
