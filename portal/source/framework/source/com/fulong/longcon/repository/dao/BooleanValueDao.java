 ...
 * @version 1.0
 */
public class BooleanValueDao extends JdbcDao {
	 /**
     * 根据内容和属性，获取属性值
     ...
     * @return String[]
     */
    public boolean[] load(String contentID, String property) throws
            SQLException {
        String SQL_SELECT =
            "select VALUE from "+QName.encode(PropertyType.BOOLEAN,property)+" where NODE_ID=? ";
        PreparedStatement command = null;
        try {

            command = connection.prepareStatement(SQL_SELECT);
            command.setString(1, contentID);
            boolean[] result = this.queryBooleanArray(command);
            return result;
    ...
    public void insert(String contentID, String property, int index,
    	...
    	String SQL_INSERT = "insert into "+QName.encode(PropertyType.BOOLEAN,property)+" (NODE_ID, VALUE, VINDEX) VALUES (?,?,?)";
        PreparedStatement command = null;
        try {
            command = connection.prepareStatement(SQL_INSERT);
            command.setString(1, contentID);
            command.setBoolean(2, value);
    ...
    public void delete(String contentID, String property) throws SQLException {
    	String SQL_DELETE_BY_ContentAndProperty =
            "delete from "+QName.encode(PropertyType.BOOLEAN,property)+" where NODE_ID=? ";
        PreparedStatement command = null;
        try {
