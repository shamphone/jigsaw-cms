 ...
 * @version 1.0
 */
public class LongProperty extends BasicProperty {
    public LongProperty(InternalRepository repository, Node content,
    ...
    protected Value[] loadValues() {
        ...
        try {
            factory.open();
            LongValueDao dao = (LongValueDao) factory.getDao(LongValueDao.class);
            long[] values = dao.load(this.node.getID(),
                                     this.definition.getID());
            Value[] ivalues = new Value[values.length];
            for (int i = 0; i < values.length; i++) {
                InternalValue svalue = new LongValue();
