...
public abstract class ServiceBaseAction extends BaseAction {
	...
	protected String uploadFile(FormFile values, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		return uploadFile(values, owner, request, response);
	}

	protected String uploadFile(FormFile values, Node owner, HttpServletRequest request, HttpServletResponse response) throws Exception {
		if (values == null || values.getFileSize() == 0){
			return null;
		}
		String fileName = FilenameUtils.getName(values.getFileName());	
		DateFormat df = new SimpleDateFormat("yyyyMMddHHmmssSSS");
		while (owner.getNodes(fileName).getSize() > 0) {		
		...
		Node newResource = owner.addNode(this.getRepository().getDefinitionManager().getDefinition("resource-scheme"), fileName);
		newResource.setProperty("resource-content", values.getInputStream());
		newResource.setProperty("mime", this.getServlet().getServletContext().getMimeType(fileName.toLowerCase()));
