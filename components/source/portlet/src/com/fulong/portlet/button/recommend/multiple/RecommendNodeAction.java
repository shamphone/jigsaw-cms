package com.fulong.portlet.button.recommend.multiple;
 ...
 * @version 2.0
 */
public class RecommendNodeAction extends PortletAction {
	/**
	 * 
	 * @param mapping
	 *            ActionMapping
		...
		for (int j = 0; IDs != null && j < IDs.length; j++) {
			...
			if (node != null) {
				for(Iterator<NodeDefinition> i = targets.iterator(); i.hasNext();){
					try {
						node.addMixinDefinition(i.next());
