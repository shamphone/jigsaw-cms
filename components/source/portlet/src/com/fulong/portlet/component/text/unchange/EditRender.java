...
public class EditRender extends FormEditRender {
	...
	public ActionForward formRender(NodeDefinition definition, ActionMapping mapping, ActionForm form,
		...
		String suggestPropId = request.getPreferences().getValue("suggestPropId", null);
		...
		if (suggestCategoryID != null && suggestCategoryID.length() > 0){
			NodeDefinition suggestDef = this.getRepository().getDefinitionManager().getDefinition(suggestCategoryID);
			request.setAttribute("category", suggestDef);
			if (suggestDef!=null && suggestPropId != null && suggestPropId.length() > 0){
				request.setAttribute("suggestProp", suggestDef.getPropertyDefinition(suggestPropId));
			}
		}	
