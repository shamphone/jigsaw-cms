...
public class SaveAction extends PortletAction {
	...
	public ActionForward action(ActionMapping mapping, ActionForm form, ActionRequest request, ActionResponse response)
			throws Exception {
		if(request.getParameter("preference(disabled)")==null)
			request.getPreferences().setValue("disabled", null);
		if(request.getParameter("preference(readonly)")==null)
			request.getPreferences().setValue("readonly", null);
