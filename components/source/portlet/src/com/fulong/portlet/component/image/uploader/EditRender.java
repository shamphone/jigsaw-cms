...
import com.fulong.longcon.repository.definition.NodeDefinitionImpl;
...
public class EditRender extends FormEditRender {
	...
	public ActionForward formRender(NodeDefinition definition, ActionMapping mapping, ActionForm form,
		...
		if(definition.getPropertyDefinition(propertyId)==null && propertyId != null){
			request.setAttribute("propertyName", propertyId+"已被删除");
		}else if (propertyId != null && propertyId.length() > 0){
			request.setAttribute("propertyName", definition.getPropertyDefinition(propertyId).getName());
		}
