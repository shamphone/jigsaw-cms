...
<table id="<bean:write name='javax.portlet.id'/>" class="<bean:write name="preferences" property="value(table-style)" ignore="true"/>" cellspacing="0" cellpadding="0" border="0">
<!-- 表头 -->
...
if (strNo == null) strNo = "0";
int pageNo = Integer.parseInt(strNo);
int count = pageNo*Integer.parseInt(rows)*Integer.parseInt(tables);
%>
<!-- 表中内容 -->
<fulong:for id="row" name="preferences" property="value(row)"><tr>
  <fulong:for id="table" name="preferences" property="value(table)">
    <logic:equal value="true" name="preferences" property="value(show-index)">
      <td align="center">
        <%=""+(count+row.intValue()*Integer.parseInt(tables)+table.intValue()+1)%>
      </td>
    </logic:equal>
    <logic:iterate id="content" name="contents" offset='<%= ""+(Integer.parseInt(tables)*row.intValue()+table.intValue())%>' length="1">
      <fulong:for id="column" name="preferences" property="value(column)">
        <td class="<%= styles[column.intValue()] %>">
