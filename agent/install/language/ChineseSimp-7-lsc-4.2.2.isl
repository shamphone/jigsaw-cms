; *** Inno Setup version 4.2.2+ Chinese Simplified messages ***
;
...
;
; $jrsoftware: issrc/Files/Default.isl,v 1.55 2004/04/29 15:56:13 jr Exp $
...
; *** Common wizard text
ClickNext=单击“下一步”继续，“取消”退出安装。
BeveledLabel=
BrowseDialogTitle=浏览目录
BrowseDialogLabel=在如下列表中选择一个目录，然后单击“确定”。
NewFolderName=新目录
...
PasswordLabel3=请输入密码，然后单击“下一步”继续。%n%n注意字母的大小写。
...
InfoBeforeClickLabel=准备好后，单击“下一步”。
WizardInfoAfter=信息
InfoAfterLabel=在安装结束之前，请先阅读下面的重要信息。
InfoAfterClickLabel=在准备好后，单击“下一步”。
...
UserInfoName=用户名称(&U)：
UserInfoOrg=公司(&O)：
UserInfoSerial=CD-KEY
UserInfoNameRequired=您必须输入一个名字。

; *** "Select Destination Directory" wizard page
WizardSelectDir=选择目标文件夹
SelectDirDesc=您准备将 [name] 安装到哪里？
SelectDirLabel3=安装程序将安装 [name] 到如下文件夹。
SelectDirBrowseLabel=继续，单击“下一步”。如果您希望选择一个不同的文件夹，单击“浏览”。
DiskSpaceMBLabel=安装本软件至少需要 [mb] MB 磁盘空间。
ToUNCPathname=安装程序无法将程序安装到一个 UNC 路径名。如果你确实需要将程序安装到网络上，必须先建立一个网络驱动器映射。
...
DirNameTooLong=文件夹名字或路径太长。
InvalidDirName=目录名无效。
BadDirName32=目录名不能包含冒号之后的任何一个字符：%n%n%1
DirExistsTitle=目录已经存在
DirExists=目录：%n%n%1%n%n已经存在。 您无论如何都要安装到此目录吗？
DirDoesntExistTitle=目录不存在
DirDoesntExist=目录：%n%n%1%n%n不存在。是否创建新目录？
...
SelectStartMenuFolderLabel3=安装程序将在如下开始菜单文件夹创建程序的快捷方式。
SelectStartMenuFolderBrowseLabel=继续，单击“下一步”。如果您希望选择一个不同的文件夹，单击“浏览”。
NoIconsCheck=不创建任何图标(&D)
MustEnterGroupName=您必须输入一个文件夹名。
GroupNameTooLong=文件夹名字或路径太长。
InvalidGroupName=文件夹名字无效。
...
CreateQuickLaunchIcon=创建快捷启动图标(&Q)
ProgramOnTheWeb=网上 %1
UninstallProgram=卸载 %1
LaunchProgram=启动 %1
AssocFileExtension=建立 %1 和文件扩展名 %2 的关联(&A)
AssocingFileExtension=建立 %1 和文件扩展名 %2 的关联...
