...
BOOL COptionGeneralPage::OnInitDialog()
{
	...
	if (ret == ERROR_SUCCESS )
	{
		char path[MAX_PATH];
		DWORD len;
		ret = RegQueryValueEx(hKey, "SME VCS", 0, 0, (unsigned char*)path, &len);
...
void COptionGeneralPage::writeIni()
{
	...
	if( ((CButton*) GetDlgItem(IDC_AUTORUN))->GetCheck() )
		RegSetValueEx(hKey, "SME VCS", 0, REG_SZ, (unsigned char*)File, strlen(File));
    else
	    ::RegDeleteValue(hKey, "SME VCS");
	::RegCloseKey(hKey);
