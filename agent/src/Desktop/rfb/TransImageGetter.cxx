...
void TransImageGetter::init(PixelBuffer* pb_, const PixelFormat& out,/*SMsgWriter* writer,*/ ColourCube* cube_)
{
  pb = pb_;
  outPF = out;
  transFn = 0;
...
void TransImageGetter::init(PixelFormat in, const PixelFormat& out, /*SMsgWriter* writer,*/ColourCube* cube_)
{
  outPF = out;
  transFn = 0;
  cube = cube_;
  ...
  if (!inPF.trueColour) {
    ...
    // CM to TC
    ColourCube cube1(6,6,6);

    if (inPF.bpp != 8)
	{}//  throw Exception("TransImageGetter: inPF has colourMap but not 8bpp");
    transFn = transSimpleFns[inPF.bpp/16][outPF.bpp/16];
    (*initSimpleCMtoTCFns[outPF.bpp/16]) (&table, inPF, &cube1,
                                          outPF);
    return;
  }
...
void TransImageGetter::setColourMapEntries(int firstCol, int nCols/*, SMsgWriter* writer*/)
{
  if (nCols == 0)
...
void TransImageGetter::translatePixels1(void* inPtr, void* outPtr,
                                       int nPixels) const
{
