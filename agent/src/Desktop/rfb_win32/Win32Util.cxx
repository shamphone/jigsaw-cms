...
LogicalPalette::LogicalPalette() : palette(0), numEntries(0) {
  ...
  if (!palette)
    printf("failed to CreatePalette", GetLastError());
}

LogicalPalette::~LogicalPalette() {
  if (palette)
    if (!DeleteObject(palette))
      printf("del palette failed", GetLastError());
}
...
PixelFormat DeviceContext::getPF(HDC dc) {
  ...
  if (!::GetDIBits(dc, bitmap, 0, 1, NULL, (BITMAPINFO*)&bi, DIB_RGB_COLORS)) {
    printf("unable to determine device pixel format", GetLastError());
  }
  if (!::GetDIBits(dc, bitmap, 0, 1, NULL, (BITMAPINFO*)&bi, DIB_RGB_COLORS)) {
    printf("unable to determine pixel shifts/palette", GetLastError());
  }
  ...
  if (format.trueColour) {
    ...
    case BI_RGB:
      ...
      case 32:
        ...
        break;
      default:
        printf("bits per pixel %u not supported", bi.bmiHeader.biBitCount);
        printf("unknown bits per pixel specified");
      };
      break;
    case BI_BITFIELDS:
      // Custom RGB layout
...
WindowDC::WindowDC(HWND wnd) : hwnd(wnd) {
  dc = GetDC(wnd);
  if (!dc)
    printf("GetDC failed", GetLastError());
}
WindowDC::~WindowDC() {
  if (dc)
    ReleaseDC(hwnd, dc);
...
CompatibleDC::CompatibleDC(HDC existing) {
  ...
  if (!dc)
    printf("CreateCompatibleDC failed", GetLastError());
}
...
BitmapDC::BitmapDC(HDC hdc, HBITMAP hbitmap) : CompatibleDC(hdc){
  ...
  if (!oldBitmap)
    printf("SelectObject to CompatibleDC failed");
}
BitmapDC::~BitmapDC() {
...
FileVersionInfo::FileVersionInfo(const TCHAR* filename) {
  ...
  if (!size)
    printf("GetVersionInfoSize failed");

  // Get version info
  buf = new TCHAR[size];
  if (!GetFileVersionInfo((TCHAR*)filename, handle, size, buf))
    printf("GetVersionInfo failed");
}
