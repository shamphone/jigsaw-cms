...
CTaskbarNotifier::CTaskbarNotifier(int Id)
{
	m_nType = 0;
	m_nId = Id;
	m_strCaption="";
	...
	m_nStartPosOffsetY = 0;
	m_nStartPosOffsetX = 0;
}
...
int CTaskbarNotifier::Create(CWnd *pWndParent)
{
	...
	return CreateEx(WS_EX_TOOLWINDOW|WS_EX_TOPMOST,strWndClass,NULL,WS_POPUP,0,0,0,0,NULL,NULL);
}
...
void CTaskbarNotifier::SetTitleRect(RECT rcTitle)
{
	m_rcTitle = rcTitle;
}
...
void CTaskbarNotifier::Show(LPCTSTR szCaption,DWORD dwTimeToShow,DWORD dwTimeToLive,DWORD dwTimeToHide,int nIncrement)
{
	...
	switch (m_nAnimStatus)
	{
		case IDT_HIDDEN:
			...
			else //if (bTaskbarOnBottom)
			{
				...
				m_dwDelayBetweenShowEvents=m_dwTimeToShow/(m_nSkinHeight/m_nIncrement);
				m_dwDelayBetweenHideEvents=m_dwTimeToHide/(m_nSkinHeight/m_nIncrement);
				m_nStartPosX=rcDesktop.right-m_nSkinWidth - m_nStartPosOffsetX;
				m_nStartPosY=rcDesktop.bottom - m_nStartPosOffsetY;
	...
	SetTimer(IDT_DESTROY, dwTimeToShow+dwTimeToLive+dwTimeToHide, NULL);
...
int CTaskbarNotifier::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_hCursor = SetCursor(AfxGetApp()->LoadCursor(IDC_LINK));
	return 0;
}
...
void CTaskbarNotifier::OnPaint()
{
	...
	dc.DrawText( this->m_strTitle, -1, m_rcTitle, DT_WORD_ELLIPSIS );
	if (m_bMouseIsOver)
	{
		dc.SetTextColor(m_crSelectedTextColor);
		pOldFont=dc.SelectObject(&m_mySelectedFont);
	}
	dc.DrawText( this->m_strCaption, -1, m_rcText, DT_WORDBREAK );
...
{
	if( m_bMouseIsOver &&  m_nAnimStatus == IDT_WAITING )
	...
	switch (nIDEvent)
	{
		...
		case IDT_DESTROY:
			{
                m_pWndParent->SendMessage(WM_TASKBARNOTIFIER_CLOSED,m_nId,0);
				return;
			}
	}
...
{
	m_pWndParent->SendMessage(WM_TASKBARNOTIFIER_CLOSED,m_nId,1);
//	this->DestroyWindow();
