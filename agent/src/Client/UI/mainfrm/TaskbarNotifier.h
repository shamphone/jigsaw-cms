...
public:
	CTaskbarNotifier(int Id);
	virtual ~CTaskbarNotifier();

	int Create(CWnd *pWndParent);
	void Show(LPCTSTR szCaption,DWORD dwTimeToShow=500,DWORD dwTimeToStay=4000,DWORD dwTimeToHide=500,int nIncrement=1);
	void Hide();
	...
	void SetTitleRect(RECT rcTitle);
	...
	int m_nStartPosOffsetY;
	int m_nStartPosOffsetX;
	int m_nStartPosX;
	int m_nStartPosY;
	int m_nCurrentPosX;
	int m_nCurrentPosY;
