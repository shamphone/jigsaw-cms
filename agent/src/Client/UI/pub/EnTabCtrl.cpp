...
//		rItem.bottom += 2;
	...
	COLORREF crFrom =  bSelected ? Lighter( RGB(140, 160, 180), 1.4f ) : RGB(216, 225, 240); 
	
	if (s_dwCustomLook & ETC_GRADIENT && pDC->GetDeviceCaps(BITSPIXEL) >= 16)
	{
		COLORREF crTo = bSelected ? Lighter( RGB(200, 210, 220), 1.4f ) : RGB(216, 225, 240);
...
COLORREF CEnTabCtrl::GetTabColor(BOOL bSelected)
{
	...
	if (bSelected && bHiliteSel)
	{
		if (bColor)
			...
			return Lighter(RGB(132, 174, 232), 1.4f);
	}
	else if (!bSelected)
	{
		if (bBackTabs || !bFlat)
		{
			if (bColor)
				...
				return Darker(RGB(200, 200, 200), 0.9f);
		}
		else
...
{
	return bSelected ? RGB( 2, 37, 131 ) : RGB( 128, 128, 128 );
}
...
BOOL CEnTabCtrl::OnEraseBkgnd(CDC* pDC)
{
	return CBaseTabCtrl::OnEraseBkgnd(pDC);
}

void CEnTabCtrl::OnPaint()
{
	CBaseTabCtrl::OnPaint();
}
