...
BOOL CPictureEx::Draw()
{
	...
	if (IsAnimatedGIF())
	{
		...
		{
			_RPTF1(_CRT_WARN, "PictureEx animation thread created, id: %#x\n", nDummy);
	...
	while (!m_bExitThread)
	{
		...
		//   3 - restore to previous
			...
			if (m_arrFrames[m_nCurrFrame].m_nDelay < 5) 
				WaitForSingleObject(m_hExitEvent, 60);
			else
...
void CPictureEx::OnPaint() 
{
	...
	if (nPaintWidth > 0)
	{
		...
		::TransparentBlt( dc.m_hDC, 0, 0, nPaintWidth, nPaintHeight, m_hMemDC, m_PaintRect.left, m_PaintRect.top, m_PaintRect.left, m_PaintRect.top, m_clrBackground);
	}
	else
	{
		::StretchBlt(dc.m_hDC, 0, 0, rc.Width(), rc.Height(), m_hMemDC, 0, 0, m_PictureSize.cx, m_PictureSize.cy, SRCCOPY);
		//::TransparentBlt( dc.m_hDC, 0, 0, m_PictureSize.cx, m_PictureSize.cy, m_hMemDC, 0,0, m_PictureSize.cx, m_PictureSize.cy, m_clrBackground);
	}
}
