...
@endverbatim
    */
     ...
     *  you must use <B>C++ delete[]</B> to delete returned point.
@verbatim
you are high recommended to use PCL_Array wrap this returned pointer. sample:
...
@endverbatim
    ...
    int GetNextFrameDelay() const {return m_nNextFrameDelay;}
     ...
     *  You must use <B>new</B> to create object and after the object be setted, you can't delete it any more, <B>frame will delete it</B>.
     *  it's a global setting, only set once at startup of program.
@verbatim
    hint : FCObjImage::SetImageHandleFactory (new FCImageHandleFactory_FreeImage) ;
@endverbatim
     */
    ...
    // Load image.
    bool Load (FCImageHandleBase* pHandler) ;
    /// Load image (determine image type by file's ext name).
    ...
    bool Save (FCImageHandleBase* pHandler, const char* szFileName, int nFlag = -1) const ;
    /**
     *  Save image (determine image type by file's ext name).
     *  nFlag depends on the image type. <BR>
     *  <B>TGA</B> : 1(use RLE compress) / -1(not use), default(not use)  <BR>
     *  <B>JPG</B> : compress quality [1..100], default(82)
     */
...
//-----------------------------------------------------------------------------
inline bool FCObjImage::Load (FCImageHandleBase* pHandler)
{
...
//-----------------------------------------------------------------------------
inline bool FCObjImage::Load (const char* szFileName)
{
...
//-----------------------------------------------------------------------------
inline bool FCObjImage::Load (BYTE* pStart, int iFileSize, IMAGE_TYPE imgType)
{
    std::auto_ptr<FCImageHandleBase>  pHandler (GetImageHandleFactory()->CreateImageHandle(imgType)) ;
    if (!pHandler.get())
        return false ;
...
//-----------------------------------------------------------------------------
inline bool FCObjImage::Save (FCImageHandleBase* pHandler, const char* szFileName, int nFlag) const
{
    ...
    bool   bRet = pHandler->SaveImage (szFileName, nFlag) ;
    assert(bRet) ;
    return bRet ;
}
//-----------------------------------------------------------------------------
inline bool FCObjImage::Save (const char* szFileName, int nFlag) const
{
