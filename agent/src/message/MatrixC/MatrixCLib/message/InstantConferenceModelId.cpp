...
string LyvcMessage::InstantConferenceModelId::toXML()
{
	...
	xml = xml + buffer;
	xml = xml + "</_conferenceid>";
	xml = xml + "<instantConferenceModelId>";
	_snprintf(buffer, BUFFERSIZE, "%I64d", this->instantConferenceModelId);
	xml = xml + buffer;
	xml = xml + "</instantConferenceModelId>";
	xml = xml + "</lyvcmessage>";
	return xml;
...
bool LyvcMessage::InstantConferenceModelId::fromXML(string& xmlString)
{
	string tagContent;
	...
	tagContent = BaseMessage::getStringBetweenTag(xmlString, "instantConferenceModelId");
	if(tagContent == ""){
		return false;
	}
	this->instantConferenceModelId = _atoi64(tagContent.c_str());
	return true;
}
