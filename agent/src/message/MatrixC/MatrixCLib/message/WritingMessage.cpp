...
string LyvcMessage::WritingMessage::toXML()
{
	...
	xml = xml + "</_conferenceid>";
	xml = xml + "<bIsWriting>";
	_snprintf(buffer, BUFFERSIZE, "%d", this->bIsWriting);
	xml = xml + buffer;
	xml = xml + "</bIsWriting>";
	xml = xml + "<receiverId>";
...
bool LyvcMessage::WritingMessage::fromXML(string& xmlString)
{
	string tagContent;
	...
	tagContent = BaseMessage::getStringBetweenTag(xmlString, "bIsWriting");
	if(tagContent == ""){
		return false;
	}
	this->bIsWriting = atoi(tagContent.c_str());
