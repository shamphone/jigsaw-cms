...
string LyvcMessage::EvaluateVideoQuality::toXML()
{
	...
	xml = xml + "</_conferenceid>";
	xml = xml + "<quality>";
	_snprintf(buffer, BUFFERSIZE, "%I32d", this->quality);
	xml = xml + buffer;
	xml = xml + "</quality>";
	xml = xml + "<receiverId>";
...
bool LyvcMessage::EvaluateVideoQuality::fromXML(string& xmlString)
{
	string tagContent;
	...
	tagContent = BaseMessage::getStringBetweenTag(xmlString, "quality");
	if(tagContent == ""){
		return false;
	}
	this->quality = atoi(tagContent.c_str());
