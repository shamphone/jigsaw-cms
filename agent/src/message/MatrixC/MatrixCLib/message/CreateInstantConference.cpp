...
string LyvcMessage::CreateInstantConference::toXML()
{
	...
	xml = xml + buffer;
	xml = xml + "</_conferenceid>";
	xml = xml + "<participants>";
	xml = xml + BaseMessage::encodeHtmlEscape(this->participants);
	xml = xml + "</participants>";
	xml = xml + "</lyvcmessage>";
	return xml;
}

bool LyvcMessage::CreateInstantConference::fromXML(string& xmlString)
{
	string tagContent;
	...
	this->participants = BaseMessage::decodeHtmlEscape(BaseMessage::getStringBetweenTag(xmlString, "participants"));
	return true;
}
