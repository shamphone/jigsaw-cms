...
string LyvcMessage::WatchUser::toXML()
{
	...
	xml = xml + "</_conferenceid>";
	xml = xml + "<viewMode>";
	_snprintf(buffer, BUFFERSIZE, "%I32d", this->viewMode);
	xml = xml + buffer;
	xml = xml + "</viewMode>";
	xml = xml + "<receiverId>";
	...
	xml = xml + buffer;
	xml = xml + "</receiverId>";
	xml = xml + "<beWatchedIds>";
	xml = xml + BaseMessage::encodeHtmlEscape(this->beWatchedIds);
	xml = xml + "</beWatchedIds>";
	xml = xml + "</lyvcmessage>";
	return xml;
}

bool LyvcMessage::WatchUser::fromXML(string& xmlString)
{
	string tagContent;
	...
	this->viewMode = atoi(tagContent.c_str());
