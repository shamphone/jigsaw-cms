...
string LyvcMessage::DefaultGroupId::toXML()
{
	...
	xml = xml + buffer;
	xml = xml + "</_conferenceid>";
	xml = xml + "<defaultGroupId>";
	_snprintf(buffer, BUFFERSIZE, "%I64d", this->defaultGroupId);
	xml = xml + buffer;
	xml = xml + "</defaultGroupId>";
	xml = xml + "</lyvcmessage>";
	return xml;
...
bool LyvcMessage::DefaultGroupId::fromXML(string& xmlString)
{
	string tagContent;
	...
	tagContent = BaseMessage::getStringBetweenTag(xmlString, "defaultGroupId");
	if(tagContent == ""){
		return false;
	}
	this->defaultGroupId = _atoi64(tagContent.c_str());
	return true;
}
