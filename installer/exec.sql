...
(
  ...
  DEFAULTPERIOD NUMBER(10),
  ITEMTYPE      NUMBER
)
;
comment on column GROUPS.DESCRIPTION
  is '暂时不用';
comment on column GROUPS.ITEMTYPE
  is '组的类型 1 是会员组，3是机构组 和membership的itemtype的定义类型相同';
alter table GROUPS
  add constraint GROUPSPRIMARYKEY primary key (ID);
create index INDEX_GROUPS_CREATETIME on GROUPS (CREATETIME);
create index INDEX_GROUPS_CREATORID on GROUPS (CREATORID);
create index INDEX_GROUPS_NAME on GROUPS (NAME);
create index INDEX_GROUPS_ORGID on GROUPS (ORGID);
create index INDEX_GROUPS_ROOT on GROUPS (ROOT);
create index INDEX_GROUPS_TYPE on GROUPS (TYPE);
create index INDEX_GROUPS_WORKFLOW on GROUPS (WORKFLOW);
...
;
comment on column NODE.DEFINITION_ID
  is '对应NODE_DEFINITION表主键';
...
create index NODE_DEFINITION_ID on NODE (DEFINITION_ID);
...
prompt
prompt Creating table PROPERTYMAPPING
prompt ==============================
prompt
...
(
  ...
  DPROID     VARCHAR2(32),
  DNODEID    VARCHAR2(32),
  REMOTESITE VARCHAR2(512),
  DPRONAME   VARCHAR2(256),
  TYPE       NUMBER,
  OWNERID    VARCHAR2(32)
)
;
comment on column PROPERTYMAPPING.SPROID
  is '源属性ID';
comment on column PROPERTYMAPPING.SNODEID
  is '源属性所在大纲ID';
comment on column PROPERTYMAPPING.DPROID
  is '目标属性ID';
comment on column PROPERTYMAPPING.DNODEID
  is '目标属性所在大纲ID';
comment on column PROPERTYMAPPING.REMOTESITE
  is '目标大纲所在网站';
comment on column PROPERTYMAPPING.DPRONAME
  is '目标属性名称';
comment on column PROPERTYMAPPING.TYPE
  is '表示投递或者接收，1为投递，2为接收';
comment on column PROPERTYMAPPING.OWNERID
  is '所有者ID';

prompt
prompt Creating table PROPERTY_CONSTRAINTS
prompt ===================================
prompt
create table PROPERTY_CONSTRAINTS
(
  PROPERTY_DEFINITION_ID VARCHAR2(128) not null,
  CONSTRAINT             VARCHAR2(128) not null,
  NODE_ID                VARCHAR2(128) not null
)
;
comment on column PROPERTY_CONSTRAINTS.PROPERTY_DEFINITION_ID
  is 'PROPERTY_DEFINITION表主键';
comment on column PROPERTY_CONSTRAINTS.CONSTRAINT
  is '数据验证表达式';
comment on column PROPERTY_CONSTRAINTS.NODE_ID
  is '某个节点定义的ID';
create index INDEX_PROPERTY_DEFINITION_ID on PROPERTY_CONSTRAINTS (PROPERTY_DEFINITION_ID);
create index PRO_CONSTRAINTS_NODE_ID on PROPERTY_CONSTRAINTS (NODE_ID);
...
prompt
prompt Creating table REFILL_LOG
prompt =========================
prompt
...
;
comment on column REFILL_LOG.PKID
  is '主键';
comment on column REFILL_LOG.USER_ID
  is '用户标识符';
comment on column REFILL_LOG.CREATION_DATE
  is '创建日期';
comment on column REFILL_LOG.POINTS
  is '充值点数';
create index REFILL_LOG_CREATION_DATE on REFILL_LOG (CREATION_DATE);
create index REFILL_LOG_PKID on REFILL_LOG (PKID);
create index REFILL_LOG_POINTS on REFILL_LOG (POINTS);
create index REFILL_LOG_USER_ID on REFILL_LOG (USER_ID);
...
;
comment on column USERS.COMMONNAME
  is '真实姓名';
...
comment on column USERS.BIRTHDAY
  is '出生日期';
comment on column USERS.REGISTERTIME
  is '注册时间';
comment on column USERS.LASTLOGIN
  is '最后登录时间';
comment on column USERS.USERNAME
  is '用户名';
comment on column USERS.AREA
  is '所在地区';
comment on column USERS.ISOPEN
  is '是否公开';
comment on column USERS.GENDER
  is '性别';
comment on column USERS.UPDATEDATE
  is '更新日期';
comment on column USERS.USERPASSWORD
  is '密码';
comment on column USERS.OPENVIEW
  is '允许其它人浏览信息';
comment on column USERS.OPENWRITE
  is '允许机构管理员修改信息';
alter table USERS
...
create index INDEX_USERS_COMMONNAME on USERS (COMMONNAME);
create index INDEX_USERS_EMAIL on USERS (EMAIL);
create index INDEX_USERS_LASTLOGIN on USERS (LASTLOGIN);
create index INDEX_USERS_REGISTERTIME on USERS (REGISTERTIME);
create index INDEX_USERS_USERPASSWORD on USERS (USERPASSWORD);
