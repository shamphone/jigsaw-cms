  ...
  <tiles:put name="dialog">
    <html:form action="insert.do">
      ...
      <table width="100%" class="sheetClass" cellpadding="2" cellspacing="0">
        <tr>
          <td>
            <div class="insetDiv" style="height:198px">
                ...
                <fulong:xtreeText>
                  <bean:define id="ID" name="categoryNode" property="ID" type="String"></bean:define>
                    <logic:equal value="true" name="category" property="enabled">
                      ...
                      <html:radio onclick="setSelect(this);" style="height:13px;" property="parentID" value="<%=ID%>" styleId="<%=ID%>"><label for="<%=ID%>"><bean:write name="categoryNode" property="name"/></label></html:radio>
                      </logic:notEqual>
                      <logic:equal value="m" name="sel">
                        <html:checkbox onclick="setSelect(this);"  style="height:13px;" property="parentID" value="<%=ID%>" styleId="<%=ID%>"><label for="<%=ID%>"><bean:write name="categoryNode" property="name"/></label></html:checkbox>
                      </logic:equal>
                    </logic:equal>
                    <logic:notEqual value="true" name="category" property="enabled">
                      ...
                      <logic:equal value="m" name="sel">
                        <html:checkbox  style="height:13px;" property="parentID" value="<%=ID%>" styleId="<%=ID%>"><label for="<%=ID%>"><bean:write name="categoryNode" property="name"/></label></html:checkbox>
                      </logic:equal>
                    </logic:notEqual>
                </fulong:xtreeText>
              </fulong:xtree>
    ...
    <tiles:put name='javascript'> 
      <script language="JavaScript" type="text/Javascript" for="document.body" event="onload()">
        tree.select();
        document.all["parentID"][0].checked=false;
        </script>
      ...
      function create(){
          if(GetRadioObject(document.forms[0].parentID)==null||GetRadioObject(document.forms[0].parentID).value==null){
              alert("请选择一个父分类！");
              return false;
          }
        var url = 'create.do?parentID='+GetRadioObject(document.forms[0].parentID).value;
        var arr = showModalDialog(url,window,"dialogWidth:246px;dialogHeight:150px;help:no;scrollbars:yes;status:no");
        if(arr!=null){
      ...
      function edit(){
        ...
        "&url="+encodeURI('../cms/definition/edit.do?ID='+GetRadioObject(document.forms[0].parentID).value);
        var arr = showModalDialog(url,window,"dialogWidth:246px;dialogHeight:150px;help:no;scrollbars:yes;status:no");
        if(arr!=null){
          var flag='<%=request.getAttribute("sel")%>';
          if(flag!='m'){
      ...
      function del(){
        if(confirm('确定删除此分类？')) {
        	var url = 'delete.do?categoryID='+GetRadioObject(document.forms[0].parentID).value;
			var oReq = new HttpRequest(encodeURI(url));
	        if (oReq.Send("GET")) {
