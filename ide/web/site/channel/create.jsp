	...
	<tr>
		...
		<fieldset><table border="0" cellpadding="2" cellspacing="2" align="center" width="100%">
			<tr>
				...
				<td><input type="text" id="name" name="name" style="IME-MODE: disabled" value="<bean:write name="name"/>"/></td>
			</tr>
			<tr>
				<td nowrap="nowrap">显示名称</td>
				<td><input type="text" id="displayName" name="displayName" value="<bean:write name="displayName"/>"  title="在页面以及客户端浏览器上显示的页面名称" /></td>
			</tr>
			<tr>
				<td>关联分类</td>
				<td colspan="2">
					<input type="hidden" id="definitionID" /> 
					<input size="26" type="text" name="definitionName" id="definitionName" readonly="readonly"/>
					<input type="button" class="btnMore" onclick="selectCategory(document.getElementById('definitionID'),document.getElementById('definitionName'))" value="选择...">
				</td>
			</tr>
			<tr>
				<td>栏目标题</td>
				<td><input type="text" onselect="setCaret(this)" onclick="setCaret(this)" onkeyup="setCaret(this)" id="title">
					<input type="button" class="btnMore"  onclick="selectProperty(document.getElementById('definitionID').value,document.getElementById('title'),['0','2','8','9','10'])" value="选择...">
				</td>
			</tr>
			<tr>
				<td>关联活动</td>
				<td>
					...
					<input type="button" class="btnMore"  onclick="doSelectActivity(document.getElementById('activityId'),document.getElementById('activityName'),oTemplate)" value="选择...">
				</td>
			</tr>
			<tr>
				<td>父文件夹</td>
				<td>
					<input type="hidden" value="<bean:write name="folderPath"/>" name="folderPath" id="folderPath" /> 
					<input type="text" value="<bean:write name="parentDisplayName"/>"  size="26" name="parentDisplayName" id="parentDisplayName" readonly="readonly"/>
		...
		<tr><td><select id="currentStyles" name="currentStyles" style="width:320px;height:120px" multiple="multiple"></select></td>	</tr>
		<tr><td><button type="button" onclick="moveOptions(this.form.allStyles, this.form.currentStyles)"> ^ 添加</button><button type="button" onclick="deleteOption(this.form.currentStyles)"> v 删除</button> <button type="button" onclick="upperShift(this.form.elements['currentStyles'])">上移</button><button type="button" onclick="lowerShift(this.form.elements['currentStyles'])">下移</button></td>	</tr>
		...
		<tr><td><select id="currentScripts" name="currentScripts" style="width:320px;height:120px" multiple="multiple"></select></td>	</tr>
		<tr><td><button type="button" onclick="moveOptions(this.form.allScripts, this.form.currentScripts)"> ^ 添加</button><button type="button" onclick="deleteOption(this.form.currentScripts)"> v 删除</button> <button type="button" onclick="upperShift(this.form.elements['currentScripts'])">上移</button><button type="button" onclick="lowerShift(this.form.elements['currentScripts'])">下移</button> </td>	</tr>
		...
		<table border="0" cellpadding="2" cellspacing="2" align="center" width="100%">
			<tr>
				<td>字符集</td>
				<td><input value="text/html; charset=utf-8" type="text" id="contentType" readonly="readonly" name="content-type" title="设定页面使用的字符集，用以说明主页制作所使用的文字已经语言，浏览器会根据此来调用相应的字符集显示page内容。"  /></td>
				</tr>
			<tr>
				<td>关键字</td>
				<td><input type="text" id="keywords" name="keywords" title="关键字设置将有利于某些常见的搜索引擎对这个页面作处理。" /></td>
				</tr>
				<tr>
				<td>作 者</td>
				<td><input type="text" id="author" title="告诉搜索引擎你的站点的制作的作者。" /></td>
			</tr>
			<tr>
				<td>爬虫设置</td>
				<td><select id="robots" title="设置搜索引擎将如何处理这个页面。">
				...
				<td>刷新设置</td>
				<td><input type="text" id="refresh" title="页面刷新时间间隔。" /></td>
			</tr>
			<tr>
				<td>到期时间</td>
				<td><input type="text" id="expires" title="页面将在这个日期后更新，设置一个比当前时间更早的时间将使用户每一次访问都需要重新获取页面。" /></td>
				</tr><tr>				
				<td>缓存设置</td>
				<td><input type="text" id="pragma" title="设定禁止浏览器从本地机的缓存中调阅页面内容，设定后一旦离开网页就无法从Cache中再调出." /></td>
			</tr>
			<tr>
				<td>进入特效</td>
				<td><select id="pageEnter" title="进入页面时的特殊效果。">
				...
				<td>离开特效</td>
				<td><select id="pageExit" title="离开页面时的特殊效果。">
					<option value="">无</option>
					<option value="progid:DXImageTransform.Microsoft.Barn(duration=2, motion='out', orientation='vertical')">打开</option>
					<option value="progid:DXImageTransform.Microsoft.Blinds(direction='down')">百叶窗</option>
					<option value="progid:DXImageTransform.Microsoft.CheckerBoard(duration=5, direction='left')">棋盘</option>
