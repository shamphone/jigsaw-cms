...
var FCKJspfProcessor = function(){
	...
	FCKRegexLib.Portlet = /(<fulong\:portlet)(.*?)(<\/fulong\:portlet>)/g ;
        ...
        if(portlets[i].className=='portletWindow'){
        	if(FCK.PorteltMap[portlets[i].id]!=null){
        		...
        		if(document.all){
        			...
        			var clone = portlets[i].cloneNode(true);
        			
        			divContainer.appendChild(clone);
        			FCK.PorteltMap[portlets[i].id].html = divContainer.innerHTML;
        			divContainer.removeChild(clone)
 ...
 * @return
 */
FCKJspfProcessor.prototype._SetNewScripts = function(data){
	if(FCK.NewScripts.Validate == false)
		return data;
	function _Replace(scriptSource){
		if(scriptSource.indexOf('portal/portal.js.jsp')>0)
			return scriptSource;
		return "";
	};
	var Reg = /<script[\s\S]*?<\/script>/gi;
	var newData = data.replace(Reg, _Replace);
	var html = "";
	for(var i=0;i<FCK.NewScripts.length;i++){
		html += '<script type="text/javascript" src="'+FCK.NewScripts[i]+'"></script>\n';
	}
	newData = newData.replace("\<\/head\>", html+"</head>");
	FCK.NewScripts.Validate = false;
	return newData;
