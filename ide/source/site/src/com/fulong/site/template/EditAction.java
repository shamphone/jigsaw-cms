 ...
 * @version 1.0
 */
public class EditAction extends TemplateBaseAction {
    ...
    protected ActionForward templateExecute(ActionMapping mapping,
        ...
        if(templateId!=null)
            siteTemplate=this.getSiteFactory(request).getTemplate(templateId);
        else
            siteTemplate=this.parseTemplate(request.getParameter("path"),request);
        form.setId(siteTemplate.getID());
        form.setName(siteTemplate.getName());
        form.setDisplayName(siteTemplate.getDisplayName());
        form.setLanguage(siteTemplate.getLocale().getLanguage());
