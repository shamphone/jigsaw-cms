 ...
 *
 */
public class RecommendAction extends ContentBaseAction {
		...
		for (int i = 0; i < ids.length; i++) {
			...
			if (definitions.length > 0) {
				for (int j = 0; j < definitions.length; j++) {
					...
					content.addMixinDefinition(newDefinition);
//					NodeEvent event = new NodeEvent(content, NodeEvent.NODE_UPDATED);
